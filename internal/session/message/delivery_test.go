...
func TestDeliver_InterruptRechecksIdleImmediatelyBeforeCtrlC(t *testing.T) {
	...
	var idleChecks atomic.Int32
	delivered := make(chan struct{}, 1)
	go RunDelivery(DeliveryConfig{
		Queue:     q,
		PtyWriter: &buf,
		IsIdle: func() bool {
			return idleChecks.Add(1) >= 2 // active at dequeue, idle immediately before delivery
		},
		OnDeliver: func() { delivered <- struct{}{} },
		Stop:      stop,
	})
	...
	if got := idleChecks.Load(); got < 2 {
		t.Fatalf("IsIdle called %d times, want a fresh pre-interrupt check", got)
	}
